...
<div class="order-details-panel">
	<div class="order-details-content">
		...
		<div class="order-details-content-body-left-panel">
			...
			%>
			<div class="order-details-content-body-left-panel-header">
				Order :
				<%=sessionOrder.getId()%></div>
			<%
				List<OrderProductJoin> orderProductJoins = OrderProductJoinLocalService
							.listOrderProductJoinsByOrderId(sessionOrder.getId());
			...
			<table class="order-details-content-body-left-panel-outer-table">
				<tbody>
					<tr>
						<td colspan="3"
							class="order-details-content-body-left-panel-outer-table-image"
							style="border: 0px solid red; width: 100px;"><img
							src="<%=product.getImageLocation()%>" /></td>
						<td>
		...
		<%
			...
			OrderTransaction orderTransaction = PaytmOrderTransactionGatewayService.createNewOrderTransaction(
					sessionUser.getId(), sessionOrder.getId(), totalPrice + (2 * gst), "INR", 1, 1);
		%>
		<div class="order-details-content-body-right-panel">
			<form action="/order/transaction?cmd=new&checkout-type=auto"
				method="POST"
				class="order-details-content-body-right-panel-form-upper">
				<script src="https://checkout.razorpay.com/v1/checkout.js"
					data-key="<%=key%>" data-amount="<%=orderTransaction.getAmount()%>"
					...
					data-buttontext="Pay with Paytm"
					data-name="Razorpay Demo" data-description="Mobile Purchase"
			...
			<table>
				<tbody>
					...
					<tr>
						<td class="product-spec-property-name">Amount</td>
						<td class="product-spec-property-value"><%=NumberFormatterUtil.getFormattedString(totalPrice)%></td>
					</tr>
					<tr>
						<td class="product-spec-property-name">GST %</td>
						<td class="product-spec-property-value">10</td>
					</tr>
					<tr>
						<td class="product-spec-property-name">CGST</td>
						<td class="product-spec-property-value"><%=NumberFormatterUtil.getFormattedString(gst)%></td>
					</tr>
					<tr>
						<td class="product-spec-property-name">SGST</td>
						<td class="product-spec-property-value"><%=NumberFormatterUtil.getFormattedString(gst)%></td>
					</tr>
					<tr>
						<td class="product-spec-property-name"></td>
						<td class="product-spec-property-value"></td>
					</tr>
					<tr>
						<td class="product-spec-property-name">Gross Amount</td>
						<td class="product-spec-property-value"><%=NumberFormatterUtil.getFormattedString(totalPrice + (2 * gst))%></td>
					</tr>
				</tbody>
			</table>
			<form action="/order/transaction?cmd=new&checkout-type=auto"
				method="POST"
				class="order-details-content-body-right-panel-form-lower">
				<script src="https://checkout.razorpay.com/v1/checkout.js"
					data-key="<%=key%>" data-amount="<%=orderTransaction.getAmount()%>"
					data-currency="<%=orderTransaction.getCurrency()%>"
					data-order_id="<%=orderTransaction.getId()%>" //This is a sample Order ID. Create an Order using Orders API. (https://razorpay.com/docs/payment-gateway/orders/integration/#step-1-create-an-order)
